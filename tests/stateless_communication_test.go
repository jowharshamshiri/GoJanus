...
func TestCommandValidationWithoutConnection(t *testing.T) {
	...
	client, err := gojanus.NewJanusClient(testSocketPath, "stateless-channel", spec)
	if err != nil {
...
func TestChannelIsolationBetweenClients(t *testing.T) {
	...
	client1, err := gojanus.NewJanusClient(testSocketPath1, "channel-1", spec)
	if err != nil {
		t.Fatalf("Failed to create client1: %v", err)
	}
	defer client1.Close()
	
	client2, err := gojanus.NewJanusClient(testSocketPath2, "channel-2", spec)
	if err != nil {
...
func TestMultiChannelAPISpecificationHandling(t *testing.T) {
	...
	client1, err := gojanus.NewJanusClient(testSocketPath, "channel-1", spec)
	if err != nil {
		t.Fatalf("Failed to create client for channel-1: %v", err)
	}
	defer client1.Close()
	
	client2, err := gojanus.NewJanusClient(testSocketPath, "channel-2", spec)
	if err != nil {
...
func TestChannelIsolationValidation(t *testing.T) {
	...
	spec := createMultiChannelAPISpec()
	client, err := gojanus.NewJanusClient(testSocketPath, "channel-1", spec)
	if err != nil {
		t.Fatalf("Failed to create client: %v", err)
	}
	// Note: SOCK_DGRAM clients are connectionless and don't need explicit cleanup
