...
func TestDirectValueHandlers(t *testing.T) {
	tests := []struct {
		...
		{
			name: "BoolHandler",
			handler: server.NewBoolHandler(func(cmd *models.JanusRequest) (bool, error) {
				return true, nil
			}),
			expected: true,
		},
		{
			name: "StringHandler",
			handler: server.NewStringHandler(func(cmd *models.JanusRequest) (string, error) {
		...
		{
			name: "IntHandler",
			handler: server.NewIntHandler(func(cmd *models.JanusRequest) (int, error) {
				return 42, nil
			}),
			expected: 42,
		},
		{
			name: "FloatHandler",
			handler: server.NewFloatHandler(func(cmd *models.JanusRequest) (float64, error) {
				return 3.14, nil
			}),
			expected: 3.14,
		},
		{
			name: "ArrayHandler",
			handler: server.NewArrayHandler(func(cmd *models.JanusRequest) ([]interface{}, error) {
		...
		{
			name: "ObjectHandler",
			handler: server.NewObjectHandler(func(cmd *models.JanusRequest) (map[string]interface{}, error) {
...
func TestCustomHandlerTypes(t *testing.T) {
	...
	handler := server.NewCustomHandler(func(cmd *models.JanusRequest) (User, error) {
...
func TestAsyncHandlers(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			handler: server.NewAsyncBoolHandler(func(cmd *models.JanusRequest) (bool, error) {
		...
		},
		{
			...
			handler: server.NewAsyncStringHandler(func(cmd *models.JanusRequest) (string, error) {
				time.Sleep(10 * time.Millisecond) // Simulate async work
...
func TestHandlerErrorHandling(t *testing.T) {
	...
	syncHandler := server.NewStringHandler(func(cmd *models.JanusRequest) (string, error) {
	...
	asyncHandler := server.NewAsyncStringHandler(func(cmd *models.JanusRequest) (string, error) {
...
func TestHandlerRegistry(t *testing.T) {
	...
	registry.RegisterHandler("test-request", handler)

	// Test retrieval
	retrievedHandler, exists := registry.GetHandler("test-request")
	if !exists {
	...
	// Test unregistration
	registry.UnregisterHandler("test-request")

	_, exists = registry.GetHandler("test-request")
	if exists {
...
// TestHandlerArguments tests handler access to request arguments
func TestHandlerArguments(t *testing.T) {
	...
	if response["original_request"] != "process-user" {
		t.Errorf("Expected original_request 'process-user', got '%v'", response["original_request"])
	}
}
